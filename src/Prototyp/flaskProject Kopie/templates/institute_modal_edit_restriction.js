...
function insertRestriction(object=false, restriction_index) {
    ...
    restrictions.forEach((restriction, index) => {
        ...
        let row = "<tr id='" + restriction['restriction_ID'] + "'><th id='course'><p> "+ course + "</p></th><th id='subject_area_code'><textarea onchange='trackRestrictionChange(this.parentElement, this.value)'>" + code + "</textarea></th><th id='incoming'>" + dropdown[0].outerHTML + "</th><th id='sub_num_mobility'><textarea style='width: 2rem' onchange='trackRestrictionChange(this.parentElement, this.value)'>"+ num_mob +"</textarea></th><th id='sub_num_months'><textarea onchange='trackRestrictionChange(this.parentElement, this.value)'>"+num_months+"</textarea></th><th><button onclick='deletion((this).parentElement.parentElement.id, `restriction`)' class='btn-delete' style='display: " + style + "'>Del</button></th></tr>";
