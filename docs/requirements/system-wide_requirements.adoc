...
*SWFA-2: Das System darf nur beschränkten Zugriff auf die Daten/das System bereitstellen, sodass nur autorisierte Mitarbeiter des Akademischen Auslandsamtes Zugriff auf das Verwaltungsfrontend haben.*
...
*NFAU-1: Das System sollte den allgemeinen Ansprüchen der HTW an die Barrierefreiheit entsprechen.*

** Zur Überprüfung siehe folgende Website: link:https://www.htw-dresden.de/hochschule/organisation/rechenzentrum/weitere-hrz-dienste/webauftritt/informationen-fuer-redakteure/barrierefreiheit-der-webseite/barrierefreie-webredaktion[
Barrierefreie Webredaktion ]


*NFAU-2: Das System sollte sowohl auf großen PC-Monitoren sowie auf kleinen Laptop Bildschirmen bedienbar sein.*
...
*NFAR-3: Das System sollte paralleles Lesen und Schreiben im Verwaltungsfrontend unterstützen. (Multi-User-System)*

** Zur Überprüfung werden zwei Loginsessions genutzt um parallel Daten zu modifizieren.
... parallel Lesen
... parallel Schreiben
... parallel den gleichen Datensatz bearbeiten

*NFAR-4: Das System sollte Konflikte durch paralleles Arbeiten an einem Datensatz verhindern.*

** Zur Überprüfung werden zwei Loginsessions genutzt um parallel Daten im selben Datensatz zu modifizieren.
...
*NFAS-1: Das Verwaltungsfrontend sollte auf verschiedenen Betriebssystemen mit unterschiedlichen Browsern laufen.*

** Zur Überprüfung wird das Verwaltungsfrontend auf den gängigsten Systemen und Browsern aufgerufen.

*NFAS-2: Ein Umzug des Datenbankservers sollte durch technisches Personal möglich sein.*

** Zur Überprüfung wird innerhalb der config Datei auf dem Webserver der Pfad zur Datenbank geändert.
...
* Die Datenbank muss als MariaDB portiert werden, um mit dem derzeitigen Informationsfrontend kompatibel zu sein und ein einfaches Einpflegen in die bestehenden Systeme des Rechenzentrums zu ermöglichen.
...
//Angaben ergänzen, nicht relevante Unterpunkte streichen oder auskommentieren
* Erweiterbarkeit muss gegeben sein
* Saubere Dokumentation 
* Datenbankstruktur
** bestehende Struktur nicht ändern
** erweitern ist möglich 

=== Rechtliche Anforderungen
//Angaben ergänzen, nicht relevante Unterpunkte streichen oder auskommentieren
* Personenbezogene Daten
** Intervallweises löschen 
** dürfen System nicht verlassen
