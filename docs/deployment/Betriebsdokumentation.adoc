...
Wie man das von dem Vorteam verwendete phpMyAdmin benutzt unnd installiert, um die Datenbank mit MySQL befehlen zu bearbeiten oder neue Datenbanken zu importieren, wird in der Betriebsdokumentation unseres Vorteams detailiert beschriebn.
Siehe: https://github.com/Rezaii-Djafari/i5-hochschulpartnerschaften/tree/master/docs/deployment[]
...
Man befindet sich nun im Home-Verzeichnis der VM und kann die Installation der Software vornehmen. 
Die Adresse muss bei Verwendung einer VM vom RZ angepasst werden. 

  

=== Installation der Server-Software 

Im Folgenden wird vorausgesetzt, dass die Installation in einer Linux Umgebung stattfindet. 
Eine Adaption auf Windows ist ohne weiters möglich, allerdings gilt dies nur für die Installation. 
Weitere Kapitel beziehen sich nur auf Linux Systeme. 
...
Eine System-Unit ermöglicht es bestimmte Skripte im Hintergrund ausführen zulassen. 
Ein System-Unit-Skript ist im geclonten Reposetory unter deploy_scripts/se_webserver.service 
...
Periodisch ist das gar nicht nötig, da das Rechenzentrum automatische Backups anlegt. Bei Änderungen durch phpMyAdmin schadet aber auch ein Backup nicht.  
