...
Eine Addaption auf Windows ist ohne weiters mögich, allerdings gitl dies nur für die Installtation.
...
=== Einführung
Die Datenbank kann mit dem Webinterface, welches unter dem Link: https://www2.htw-dresden.de/~aaapartnerhs/phpmyadmin/ zuereichen ist, unabhängig zum Verwaltungsfrontend portiert, editiert und gebackuped werden.
...
Nach dem Login werden in dem linken Reiter die verfügbaren Datenbanken angezeigt.
...
=== Was ist zu tun wenn das Verwaltungsfrontend nicht erreichbar ist obwolh der Server läuft?
...
... Installieren sie gegebenfalls die mindest Python Version
.. *Nein*: siehe nächster Punkt.

=== Wie kann man eine vorhandene Systemunit configurieren?
. Die Systemunit abmelden mit 
. Die Systemunit ändern (im Beispiel mit Vim)
. dann diese wieder regestrieren und evtl. starten
[source, bash]
----
sudo systemctl disable se_webserver.service
...
Ja, das Rechenzetrum macht periodisch Backups.
...
. Häckchen aus Struktur entfernen
