...
Als Webserver dient der www2-Server des Rechenzentrums, auf welchem ein Webspace des Funktionsaccountes liegt. Dort läuft das Backend in (...) und das Frontend mithilfe von (...)

==== Authentifizierung im Browser

Die Authentifizierung erfolgt grundlegend über die `$_SESSION`-Variable in PHP. Der Nutzer loggt sich zunächst auf der Login-Seite ein - wenn die Zugangsdaten dem Datenbank-Nutzer gleichen, ist dieser erfolgreich authentifiziert. Es existiert keine seperate Nutzerdatenverwaltung.

Nach dem Login-Vorgang wird der Nutzer auf die Startseite weitergeleitet. Für alle weiteren Seiten wird erst überprüft, ob in der `$_SESSION`-Variable ein Nutzername und Passwort hinterlegt sind und erst dann wird ein Verbindungsaufbau versucht. 

Somit verhindert man direkten Zugriff auf einzelne Seiten, indem der Nutzer bei einem nicht erfolgten Zugriff auf die Datenbank zur Login-Seite weitergeleitet wird.
...
Für die Darstellung wird generell eine Mischung aus (...) und (...) genutzt.

Die Listen von Hochschulen, Mentoren, Ländern, usw. werden beim Aufruf der Seite durch das PHP-Skript geladen und zusätzlich bei der Auswahl eines Filters aktualisiert. Sie sind zur besseren Übersicht in einer Tabelle angeordnet.

Das Bearbeitungsformular wird als neues Fenster über dem Inhalt der Seite dargestellt. Dieses ist am Anfang des Seitenaufrufes versteckt und wird nur angezeigt, wenn der Bearbeiten- oder Erstellen-Button betätigt wurde.
